...
function test_data_load() {
  ...
  function lesson_add(target, num) {
    ...
    for (let i = 0; i < num; i++) {
      qs(target).children[1].children[0].click();
    }
  }
  ...
  function ele_lesson_add(cls_num, offset, lesson_num, continuity_num) {
    ...
    qs("#elective_lesson_list_" + cls_num + "_" + offset).children[2].value =
      lesson_num;
    qs("#elective_lesson_list_" + cls_num + "_" + offset).children[3].value =
      continuity_num;
  ...
  function t_f_les(target_Z, x, y, n) {
    for (let i = y; i < n + y; i++) {
      qs("#timetable_" + target_Z)
        .querySelectorAll("tr")[i].querySelectorAll("td")[x].querySelector("li")
        .click();
    }
  }
  ...
  function t_click_ele_lesson(cls_num, lsn_num, ele_flg = false) {
    ...
    //ele_flg:選択授業の時
    cls_num++;
    let lsn = document.querySelector("#timetable_link_sidebar").children[
      cls_num
    ];
    if (ele_flg) {
      //選択授業の時
      lsn
        .querySelector(".timetable_tab_elective_lesson")
        .children[lsn_num].click();
    } else {
      //通常授業の時
      lsn
        .querySelector(".timetable_tab_normal_lesson")
        .children[lsn_num].click();
    }
  }
