...
function update_lesson_list(par) {
    ...
    if (type == "normal") {
        // log([suf1, suf2, suf3]);
        // log(normal_lesson_list[suf1][suf2]);
        normal_lesson_list[suf1][suf2][0] = par.children[0].value;
        normal_lesson_list[suf1][suf2][1] = par.children[1].selectedIndex > -1 ? par.children[1].options[par.children[1].selectedIndex].value : "";
        normal_lesson_list[suf1][suf2][2] = par.children[2].selectedIndex > -1 ? par.children[2].options[par.children[2].selectedIndex].value : "";
    } else if (type == "elective") {
        elective_lesson_list[suf1][suf2][suf3][0] = par.children[0].value;
        elective_lesson_list[suf1][suf2][suf3][1] = par.children[1].selectedIndex > -1 ? par.children[1].options[par.children[1].selectedIndex].value : "";
        elective_lesson_list[suf1][suf2][suf3][2] = par.children[2].selectedIndex > -1 ? par.children[2].options[par.children[2].selectedIndex].value : "";
    }
}
...
function init_add_event(val, list, arr, change_event_name, deleat_event_name) {
    ...
    div.innerHTML =
        "<input value=" + val + ">" +
        "<button onclick=" + change_event_name + "(this)>変更</button>" +
        "<button onclick=" + deleat_event_name + "(this)>削除</button>";
...
function normal_lesson_add_event(val, parent) {
    ...
    div.innerHTML =
        ...
        '<input type="button" value="o" onclick="get_lesson_num_btn(this)">' +
...
function teacher_add_btn() {
    init_add_event(teacher_name_text.value, teacher_list_dom, teacher_list, "teacher_change_btn", "teacher_deleat_btn");
    select_add("teacher_select", teacher_name_text.value);
    teacher_name_text.value = ""; //textを初期化
}

function room_add_btn() {
    init_add_event(room_name_text.value, room_list_dom, room_list, "room_change_btn", "room_deleat_btn");
...
function elective_lesson_list_add_btn(e) {
    let parent = parent_class_search(e, "lesson_list");
    elective_lesson_list_add_event(getIdNum(parent), parent);
}
...
function elective_lesson_list_add_event(val, parent) {
    ...
    //HTML成形
    let div = creEle("div");
    div.id = "elective_lesson_list_" + val + "_" + (elective_lesson_list[val].length - 1);
...
function elective_lesson_add_event(val1, val2, parent) {
    ...
    div.innerHTML =
        ...
        '<input type="button" value="o" onclick="get_lesson_num_btn(this)">' +
        '<input type="button" value="変更" onclick="change_lesson(this)">' +
        '<input type="button" value="削除" onclick="elective_lesson_deleat_btn(this)">';
    div.getElementsByClassName("teacher_select")[0].innerHTML = selector_create(teacher_list).innerHTML;
    div.getElementsByClassName("room_select")[0].innerHTML = selector_create(room_list).innerHTML;
...
function class_deleat_btn(e) {
    ...
    for (let i = 0; i < ret.length; i++) {
        ...
        let elective_lesson = ret[i].getElementsByClassName("elective_lesson_list_list")[0];
...
function elective_lesson_list_deleat_btn(e) {
    let parent = parentDom(parentDom(e));
    let big_parent = parentDom(parent);
    lesson_deleat_event(parentDom(e), elective_lesson_list[getIdNum(parent, 0)], 1, getIdNum(parent, 0));
